...
def format_monte_carlo_summary(probability: float, macro_context: Optional[Dict[str, Any]] = None) -> Dict[str, str]:
    """
    ...
    adj_note = (
        f" Macro-adjusted confidence: **{adj:.0%}**."
...
def build_insight_cards(
    ...
    macro_score = macro_context.get("macro_context_score", 1.0) if macro_context else 1.0
    int_trend = macro_context.get("interest_rate_trend", "stable") if macro_context else "stable"
    ...
    if category.lower() == "conservative":
        ...
        risk_rec = "Hold position. A 60/40 equity-to-debt mix is ideal for your profile."
    ...
    if probability > 80:
        ...
        goal_rec = "Hold your position and consider a 10% SIP increase for extra security."
    else:
...
def build_scenario_projections(
    ...
    """
    scenarios = [
        ("Best Case",     0.15, "green",  "🌟"),
        ("Expected Case", 0.12, "yellow", "📊"),
        ("Worst Case",    0.08, "red",    "⚠️"),
    ]
    ...
    for name, annual_rate, colour, icon in scenarios:
        ...
        if r == 0:
            ...
            fv = existing_corpus * ((1 + r) ** months) + monthly_sip * (((1 + r) ** months - 1) / r)
